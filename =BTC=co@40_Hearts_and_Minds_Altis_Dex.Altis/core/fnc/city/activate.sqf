...
if (count _data_units > 0) then {
	...
	_ratio = (switch _type do {
		case "Hill" : {0.6}; //default 0.6
		case "NameLocal" : {0.75}; //default 0.75
		case "NameVillage" : {1}; //default 1
		case "NameCity" : {2};
	...
	if (_type != "Hill") then {
		...
		_factor = (switch _type do {
			case "NameLocal" : {0.5}; //default 0.5
			case "NameVillage" : {1.5}; //default 1.5
			case "NameCity" : {3};
			case "NameCityCapital" : {6};
			case "Airport" : {1.5};
			default {1}; //default 1
			});
		_n = 9 * _factor; //default 3 * _factor
